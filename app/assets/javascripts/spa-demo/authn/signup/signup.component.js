(function() {
  ...
  SignupController.$inject = ["$scope","$state",
                              ...
                              "spa-demo.subjects.Image","$q","$window"];
  function SignupController($scope, $state, Authn, DataUtils, Image, $q,$window) {
    var vm=this;
    vm.signupForm = {}
    vm.signup = signup;
      ...
      $scope.signup_form.$setPristine();
          ...
          Authn.signup(vm.signupForm).then(
            function(response){
              ...
              if (vm.signupForm.addImage&&vm.item.image_content!=null) {
                vm.item.$save().then(
                  function() {
